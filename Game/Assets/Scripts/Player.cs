...
public class Player : MonoBehaviour
{
		...
		if (!isDead)
		{
			...
			if (!crouched && !lookingUp && !reloading)
			{
				...
				if (Input.GetKeyDown(UserPrefs.keys["Right"]))
					hForce = 1;
				if (Input.GetKeyUp(UserPrefs.keys["Right"]))
					hForce = 0;
				if (Input.GetKeyDown(UserPrefs.keys["Left"]))
					hForce = -1;
				if (Input.GetKeyUp(UserPrefs.keys["Left"]))
					hForce = 0;
			...
			//Input.GetKeyDown(UserPrefs.keys[""])
			if (Input.GetKeyDown(UserPrefs.keys["Jump"]) && onGround && !reloading)
			{
				jump = true;
			}
			else if (Input.GetKeyUp(UserPrefs.keys["Jump"]))
			{
			...
			if (Input.GetKeyDown(UserPrefs.keys["Fire1"]) && Time.time > nextFire && bullets > 0 && !reloading && canFire)
			{
			...
			else if (Input.GetKeyDown(UserPrefs.keys["Fire1"]) && bullets <= 0 && onGround)
			{
				StartCoroutine(Reloading());
			}

			lookingUp = Input.GetKey(UserPrefs.keys["Up"]);
			crouched = Input.GetKey(UserPrefs.keys["Down"]);

			anim.SetBool("LookingUp", lookingUp);
			anim.SetBool("Crouched", crouched);

			if (Input.GetKeyDown(UserPrefs.keys["Reload"]) && onGround)
			{
				StartCoroutine(Reloading());
			}

			if (Input.GetKeyDown(UserPrefs.keys["Fire2"]) && bombs > 0)
			{
	...
	private void OnCollisionEnter2D(Collision2D other)
	{
		...
		else if (other.gameObject.CompareTag("EnemyBullets"))
		{
			Destroy(other.gameObject);
			StartCoroutine(TookDamage());
		}
